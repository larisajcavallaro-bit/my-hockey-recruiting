...
export default function CoachPlayerDetailPage() {
  ...
  useEffect(() => {
    if (!player?.parentId || !coachProfileId) return;
  ...
  }, [player?.id, player?.parentId, coachProfileId]);

  const handleRequestContact = async () => {
    if (!player || !coachProfileId) return;
    setRequesting(true);
    try {
  ...
  const handleBlockUser = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push("/coach-dashboard/players");
      } else {
  ...
  const email = player.parent?.user?.email ?? "";
  const phone = player.parent?.phone ?? "";
  const rating = player.rating ?? 0;
  const reviewCount = player.reviewCount ?? 0;
  const reviews = player.reviews ?? [];
  const hasContactAccess = contactStatus === "approved";
      ...
      <Card className="overflow-hidden border-none bg-secondary-foreground/60 dark:bg-slate-900/50">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 md:p-8">
          ...
          <div className="md:col-span-2 space-y-6">
            <div>
              <h1 className="text-4xl font-extrabold text-sub-text3">{player.name}</h1>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="bg-white dark:bg-slate-800 p-3 rounded-xl shadow-sm">
                <p className="text-xs text-muted-foreground uppercase">Birth Year</p>
            ...
            {/* Contact Information - hidden until approved */}
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-6 text-center text-sub-text3">
                {(player.position === "Forward" || player.position === "Defense" || !player.position) && (
                  <>
                    ...
                    <div className="space-y-2 bg-secondary-foreground/50 rounded-2xl p-3 flex flex-col items-center justify-center">
                      ...
                      <div>
                        <p className="text-3xl font-bold">{player.assists ?? 0}</p>
                    ...
                    <div className="space-y-2 bg-secondary-foreground/50 rounded-2xl p-3 flex flex-col items-center justify-center">
                      ...
                      <div>
                        <p className="text-3xl font-bold">{player.plusMinus ?? "-"}</p>
                ...
                {player.position === "Goalie" && (
                  <>
                    <div className="space-y-2 bg-secondary-foreground/50 rounded-2xl p-3 flex flex-col items-center justify-center">
                      ...
                      <div>
                        <p className="text-3xl font-bold">{player.gaa ?? "-"}</p>
                        <p className="text-xs uppercase">GAA</p>
                      </div>
                    </div>
                    <div className="space-y-2 bg-secondary-foreground/50 rounded-2xl p-3 flex flex-col items-center justify-center">
                      <PercentCircle className="w-5 h-5 opacity-70" />
                      <div>
                        <p className="text-3xl font-bold">{player.savePct ?? "-"}</p>
                        <p className="text-xs uppercase">Save %</p>
                      </div>
                    </div>
                  </>
                )}
        ...
        <TabsContent value="reviews" className="space-y-4">
          {reviews.length > 0 ? (
            reviews.map((review, idx) => (
              <Card className="bg-secondary-foreground/60 text-sub-text3" key={idx}>
