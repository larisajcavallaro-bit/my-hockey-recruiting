...
interface AdminAddFacilityModalProps {
  onAdded: () => void;
}

export default function AdminAddFacilityModal({ onAdded }: AdminAddFacilityModalProps) {
  const [open, setOpen] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
    defaultValues: { amenities: [], website: "", facilityType: undefined, imageUrl: "" },
  });
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      toast.success("Training added. It’s live on the website now.");
      form.reset({ amenities: [], website: "", facilityType: undefined, imageUrl: "" });
      setOpen(false);
      onAdded();
    } catch {
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-slate-800 border-slate-700">
        <div className="mb-6">
          ...
          <DialogDescription className="text-slate-400 text-sm mt-1">
            Create a new training location. It will appear on the public Training page immediately.
          </DialogDescription>
        </div>
